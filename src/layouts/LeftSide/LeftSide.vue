...
<template>
  <q-drawer show-if-above bordered :width="$q.platform.is.mobile?300:400">
    <div class="q-gutter-sm row q-pa-none full-width">
      <q-tabs v-model="tab" no-caps outside-arrows mobile-arrows active-bg-color="primary" :active-color="$q.dark.isActive?`black`:`white`" indicator-color="yellow-3" class="text-primary shadow-4 full-width">
    ...
    <q-scroll-area class="" style="height:calc(100% - 100px)">
      <q-tab-panels keep-alive v-model="tab" animated>
        <q-tab-panel name="room" class="q-pa-none">
          <q-list>
            ...
            <q-expansion-item dense :header-class="$q.dark.isActive? 'left-header-dark' : 'left-header-light'" default-opened :label="$t('light')">
              <Light />
            </q-expansion-item>
          </q-list>
        </q-tab-panel>
        ...
        <q-tab-panel name="screen" class="q-pa-none">
          <q-list>
            <q-expansion-item dense :header-class="$q.dark.isActive? 'left-header-dark' : 'left-header-light'" default-opened :label="$t('screenShape')">
              <ScreenSettings />
            </q-expansion-item>
            ...
            <q-expansion-item dense :header-class="$q.dark.isActive? 'left-header-dark' : 'left-header-light'" default-opened :label="$t('ScreenMap')">
              <ScreenMap />
            </q-expansion-item>
          </q-list>
        </q-tab-panel>
      ...
      <div class="row fixed-bottom" style="justify-content: right;height:50px ; border-top:1px solid #ccc">
        ...
        <q-btn flat v-show="tab === 'screen'" :label="`← ${$t('previous')}`" @click="tab = 'room'" text-color="primary" />
        <q-btn flat v-show="tab === 'screen'" :label="`${$t('next')} →`" @click="tab = 'projector'" text-color="primary" />

        <q-btn flat v-show="tab === 'projector'" :label="`← ${$t('previous')}`" @click="tab = 'screen'" text-color="primary" />
        <q-btn flat v-show="tab === 'projector'" :label="`${$t('next')} →`" @click="tab = 'result'" text-color="primary" />

        <q-btn flat v-show="tab === 'result'" :label="`← ${$t('previous')}`" @click="tab = 'projector'" text-color="primary" />
        <q-btn flat v-show="tab === 'result'" :label="`${$t('exportPdf')}`" icon="picture_as_pdf" @click="showDlgPDF = true" text-color="primary" />
      </div>
